...
- `slides/scripts/lint-slides.mjs` — the content linter. "conference"/"Konferenz" are forbidden event words; the one exception is the product name `clash-conference` as a whole token (a lookbehind), so prose says "talk" or "programme".
- `../clash-conference` (`https://github.com/agilino/clash-conference.git`) — a second repository for task 19, cloned next to the CLASH clone. Published once the app is finished; until then it holds no app and no `19-start` branch. Once out, its `main` is the finished app, its `19-start` is the app without `app/api/publish/route.ts` and `lib/clash-agent.ts`, its `19-solution` the app with the finished route. Not built by `scripts/prepare-branches.sh`. CLASH's own `19-start` is not `14-start`: it carries the MCP starter (`workshop-artifacts/19-build-mcp/server.start.ts`), and CLASH's `19-solution` the finished server — both generated by `scripts/prepare-branches.sh`, never by hand.
...
10. **Name the repository and the app every time.** Task 19 works in two clones at once, so
    ...
    have a branch called `19-start` and one called `19-solution`; never write either without
    saying which one.
